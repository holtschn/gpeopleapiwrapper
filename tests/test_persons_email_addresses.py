...
class TestPersonWrapperEmailAddresses(tpb.FixtureMixin, unittest.TestCase):
    ...
    def test_read_email_addresses_attributes(self):
        person = TestPersonWrapperEmailAddresses.read_fixture_tester_klaus()
        ...
        self.assertEqual("k.tester@example.de", email_addresses_work.value)
    ...
    def test_read_all_email_address_values(self):
        ...
        self.assertIn("k.tester@example.de", email_address_values)
    ...
    def test_update_email_addresses_value(self):
        ...
        self.assertEqual("[Updated] k.tester@example.de", person.email_addresses.first_of_type("work").value)
    ...
    def test_remove_email_address(self):
        ...
        person.email_addresses.remove_by_value("k.tester@example.de")

        self.assertEqual(1, len(list(person.email_addresses.all())))
        self.assertEqual("klaus.tester@example.com", person.email_addresses.first().value)

        self.assertTrue(person.has_changes())

    def test_append_email_address_to_unset_attribute(self):
        person = TestPersonWrapperEmailAddresses.read_fixture_tester_blank()
        self.assertFalse(person.has_changes())
